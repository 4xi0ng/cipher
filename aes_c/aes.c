...
* AES - CFB
*******************/
int aes_encrypt_cfb(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[16];
	BYTE * sub_buf_out[16];
	int num = 16;
	...
	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * num], num);

		aes_encrypt(iv_buf, buf_out, key, keysize);
		memcpy(sub_buf_out, buf_out, num);
		xor_buf(buf_in, sub_buf_out, num);

		//save ciphertext
		memcpy(&out[idx * num], sub_buf_out, num);
		
		//create new iv
		memcpy(iv_buf, &iv_buf[num], AES_BLOCK_SIZE-num);
		memcpy(&iv_buf[AES_BLOCK_SIZE - num], sub_buf_out, num);
	}

	return(TRUE);
}

int aes_decrypt_cfb(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[16];
	BYTE * sub_buf_out[16];
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		//create new iv
		memcpy(iv_buf, &iv_buf[num], AES_BLOCK_SIZE - num);
		memcpy(&iv_buf[AES_BLOCK_SIZE - num], buf_in, num);//加密时使用密文产生输出，解密同样应使用密文
	}

	return(TRUE);
}

int aes_encrypt_cfb8(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[1];
	BYTE * sub_buf_out[1];
...
int aes_decrypt_cfb8(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[1];
	BYTE * sub_buf_out[1];
	...
	for (idx = 0; idx < blocks; idx++) {
		...
		//create new iv
		memcpy(iv_buf, &iv_buf[num], AES_BLOCK_SIZE - num);
		memcpy(&iv_buf[AES_BLOCK_SIZE - num], buf_in, num);
	}
...
* AES - OFB
*******************/
int aes_encrypt_ofb(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[16];
	BYTE * sub_buf_out[16];
...
int aes_encrypt_ofb8(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	BYTE * buf_in[1];
	BYTE * sub_buf_out[1];
